<?php
/**
 * Le pont entre FamilyShop et le portail NeedHelpApp.
 *
 * Copié depuis teaching, où il a fait ses preuves. Deux différences
 * seulement : le code d'application, et le fait que la table `users`
 * d'ici ne porte que trois colonnes utiles — l'identité vit au portail.
 *
 * FamilyShop ne gère plus d'identité : c'est le portail qui sait qui est
 * connecté, et son cookie « nha_session », posé sur .needhelpapp.com, vaut
 * pour tous les sous-domaines.
 *
 * Mais familyshop garde sa table `users`. Ce n'est pas un doublon : toutes ses
 * données — dictées, listes, séries, résultats — y sont rattachées par une
 * clé étrangère. Cette table devient donc un simple point d'ancrage local,
 * relié au compte central par `account_id`.
 *
 * Un compte du portail qui arrive pour la première fois voit sa ligne locale
 * créée à la volée. Un compte familyshop antérieur au portail est retrouvé par
 * son adresse : rien n'est perdu, et rien n'est dupliqué.
 */

declare(strict_types=1);

/* Le code de l'application, posé dès le chargement du fichier.
   « SetEnv NHA_APP familyshop » dans le .htaccess ne parvient pas toujours à
   PHP — c'est fréquent en PHP-FPM. Sans ce garde-fou, les droits seraient
   lus pour « portail » et l'abonnement paraîtrait absent. */
if (!getenv('NHA_APP')) {
    putenv('NHA_APP=familyshop');
    $_ENV['NHA_APP'] = 'familyshop';
    $_SERVER['NHA_APP'] = 'familyshop';
}
 ...
 * familyshop doit rester consultable plutôt que de rendre une page blanche.
 */
function nhaCompte(): ?array
{
    ...
    try {
        ...
        error_log('[familyshop] portail injoignable : ' . $e->getMessage());
        return $memo = null;
    }
}
 ...
 * dans familyshop avant la mise en place du portail.
 */
function nhaUtilisateurLocal(array $compte): ?array
{
    ...
    try {
        ...
        // 2. un compte familyshop antérieur porte la même adresse : on le relie
        if (!$u) {
            ...
            if ($u) {
                ...
                error_log('[familyshop] compte existant rattaché au portail : ' . $email);
            }
        }
        ...
        error_log('[familyshop] rattachement impossible : ' . $e->getMessage());
        return null;
    }
}
...
function nhaAbonnement(int $accountId): array
{
    ...
    try {
        return nha_entitlement($accountId, 'familyshop');
    } catch (Throwable $e) {
        error_log('[familyshop] droits illisibles : ' . $e->getMessage());
        return ['plan' => 'gratuit', 'status' => 'aucun', 'until' => null];
    }
}

/* =====================================================================
   S'inscrire et se connecter DEPUIS familyshop
...
function nhaConnexion(string $email, string $motDePasse): array
{
    ...
    try {
        ...
        nha_attach_app((int) $compte['id'], 'familyshop');
        nha_log((int) $compte['id'], 'login', 'familyshop');
        return ['ok' => true, 'compte' => $compte];
    } catch (Throwable $e) {
        error_log('[familyshop] connexion au portail impossible : ' . $e->getMessage());
        return ['ok' => false, 'erreur' => nhaRaison('service momentanément indisponible', $e)];
    }
}

/**
 * Crée un compte central depuis familyshop et ouvre la session.
...
function nhaInscription(string $email, string $motDePasse, string $nom): array
{
    ...
    try {
        ...
        error_log('[familyshop] inscription au portail impossible : ' . $e->getMessage());
        return ['ok' => false, 'erreur' => nhaRaison('service momentanément indisponible', $e)];
    }
}
...
function nhaGoogle(string $sub, string $email, bool $verifie, string $nom): array
{
    ...
    if ($sub === '' || $email === '') {
        error_log('[familyshop] appel Google incomplet : sub="' . $sub . '" email="' . $email . '"');
    ...
    try {
        ...
        error_log('[familyshop] connexion Google impossible : ' . $e->getMessage());
        return ['ok' => false, 'erreur' => nhaRaison('service momentanément indisponible', $e)];
    }
}
...
{
    $retour = 'https://' . ($_SERVER['HTTP_HOST'] ?? 'familyshop.needhelpapp.com') . '/';

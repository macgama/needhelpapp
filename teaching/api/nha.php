<?php
/**
 * Le pont entre teaching et le portail NeedHelpApp.
 *
 * Teaching ne gère plus d'identité : c'est le portail qui sait qui est
 * connecté, et son cookie « nha_session », posé sur .needhelpapp.com, vaut
 * pour tous les sous-domaines.
 *
 * Mais teaching garde sa table `users`. Ce n'est pas un doublon : toutes ses
 ...
 * créée à la volée. Un compte teaching antérieur au portail est retrouvé par
...
/* Le code de l'application, posé dès le chargement du fichier.
   « SetEnv NHA_APP teaching » dans le .htaccess ne parvient pas toujours à
   PHP — c'est fréquent en PHP-FPM. Sans ce garde-fou, les droits seraient
   lus pour « portail » et l'abonnement paraîtrait absent. */
if (!getenv('NHA_APP')) {
    putenv('NHA_APP=teaching');
    $_ENV['NHA_APP'] = 'teaching';
    $_SERVER['NHA_APP'] = 'teaching';
}
 ...
 * teaching doit rester consultable plutôt que de rendre une page blanche.
 */
function nhaCompte(): ?array
{
    ...
    try {
        ...
        error_log('[teaching] portail injoignable : ' . $e->getMessage());
        return $memo = null;
    }
}
 ...
 * dans teaching avant la mise en place du portail.
 */
function nhaUtilisateurLocal(array $compte): ?array
{
    ...
    try {
        ...
        // 2. un compte teaching antérieur porte la même adresse : on le relie
        if (!$u) {
            ...
            if ($u) {
                ...
                error_log('[teaching] compte existant rattaché au portail : ' . $email);
            }
        }
        ...
        error_log('[teaching] rattachement impossible : ' . $e->getMessage());
        return null;
    }
}
...
function nhaAbonnement(int $accountId): array
{
    ...
    try {
        return nha_entitlement($accountId, 'teaching');
    } catch (Throwable $e) {
        error_log('[teaching] droits illisibles : ' . $e->getMessage());
        return ['plan' => 'gratuit', 'status' => 'aucun', 'until' => null];
    }
}

/* =====================================================================
   S'inscrire et se connecter DEPUIS teaching
...
function nhaConnexion(string $email, string $motDePasse): array
{
    ...
    try {
        ...
        nha_attach_app((int) $compte['id'], 'teaching');
        nha_log((int) $compte['id'], 'login', 'teaching');
        return ['ok' => true, 'compte' => $compte];
    } catch (Throwable $e) {
        error_log('[teaching] connexion au portail impossible : ' . $e->getMessage());
        return ['ok' => false, 'erreur' => nhaRaison('service momentanément indisponible', $e)];
    }
}

/**
 * Crée un compte central depuis teaching et ouvre la session.
...
function nhaInscription(string $email, string $motDePasse, string $nom): array
{
    ...
    try {
        ...
        error_log('[teaching] inscription au portail impossible : ' . $e->getMessage());
        return ['ok' => false, 'erreur' => nhaRaison('service momentanément indisponible', $e)];
    }
}
...
function nhaGoogle(string $sub, string $email, bool $verifie, string $nom): array
{
    ...
    if ($sub === '' || $email === '') {
        error_log('[teaching] appel Google incomplet : sub="' . $sub . '" email="' . $email . '"');
    ...
    try {
        ...
        error_log('[teaching] connexion Google impossible : ' . $e->getMessage());
        return ['ok' => false, 'erreur' => nhaRaison('service momentanément indisponible', $e)];
    }
}
...
{
    $retour = 'https://' . ($_SERVER['HTTP_HOST'] ?? 'teaching.needhelpapp.com') . '/';
